...
namespace OpenDis.Core
{
    ...
    public class CoordinateConversions
    {
        ...
        public static double[] xyzToLatLonRadians(double[] xyz)
        {
            ...
            double tanPhi =
                (z + (ePrimeSquared * b * (Math.Pow(Math.Sin(BZero), 3)))) /
            ...
            rSubN =
                ...
                Math
                    .Sqrt((
                    (a * a) * (Math.Cos(phi) * Math.Cos(phi)) +
                    ((b * b) * (Math.Sin(phi) * Math.Sin(phi)))
                    ));
        ...
        private void FindMetersPerLat(float lat) // Compute lengths of degrees
        {
            ...
            metersPerLat =
                m1 +
                (m2 * Mathf.Cos(2 * (float) lat)) +
                (m3 * Mathf.Cos(4 * (float) lat)) +
                (m4 * Mathf.Cos(6 * (float) lat));
            metersPerLon =
                (p1 * Mathf.Cos((float) lat)) +
                (p2 * Mathf.Cos(3 * (float) lat)) +
                (p3 * Mathf.Cos(5 * (float) lat));
        }
        ...
        )
        {
            double[] degrees =
                CoordinateConversions
                    .getXYZfromLatLonRadians(latitude *
                    CoordinateConversions.DEGREES_TO_RADIANS,
